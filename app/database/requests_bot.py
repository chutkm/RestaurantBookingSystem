...
from .models import async_session, Clients, Reservations, Tables, Restaurants, Menu, Preorder, Promotions
from sqlalchemy import select, update, delete, func, or_, and_, Integer
...
async def all_rest_view():
    async with async_session() as session:
        result = await session.execute(select(Restaurants.restaurant_id, Restaurants.address))
